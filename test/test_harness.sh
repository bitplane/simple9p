...
test_disconnect() { # (Added more logging for clarity)
...
test_gather() { # (Ensured paths are clear, generic headers)
    ...
    echo "--- stdout ---"; cat "./${target_name}.stdout" 2>/dev/null || echo "(stdout $target_name N/A)"
    echo "--- stderr ---"; cat "./${target_name}.stderr" 2>/dev/null || echo "(stderr $target_name N/A)"
    echo "--- return_code ---"; cat "./${target_name}.return" 2>/dev/null || echo "(return $target_name N/A)"
    if [[ -d "$target_data_dir" ]]; then
        echo "--- directory_listing (content of data directory) ---"
        ( cd "$target_data_dir" || { echo "ERROR: Cannot cd $target_data_dir"; exit 1; }
          find . -print0 | sort -z | while IFS= read -r -d $'\0' entry; do
            ...
            echo "Path: $clean_entry"; stat_output=$(stat "$entry" 2>/dev/null)
            if [[ $? -eq 0 ]]; then echo "$stat_output" | sed -e 's/Inode: [0-9]\+//g' -e 's/Device: [0-9a-fA-F]*h\/[0-9]*d//g' -e 's/Links: [0-9]*//g' -e 's/[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\} [0-9]\{2\}:[0-9]\{2\}:[0-9]\{2\}\(\.[0-9]*\)\{0,1\} [+-][0-9]\{4\}//g' -e 's/Birth: -//g' -e 's/Birth: [0-9]\{4\}.*//g' -e '/^[[:space:]]*$/d' -e 's/Access: [0-9]\{4\}.*//g' -e 's/Modify: [0-9]\{4\}.*//g' -e 's/Change: [0-9]\{4\}.*//g';
            else echo "Stat failed for $clean_entry"; fi
            if [[ -f "$entry" && ! -L "$entry" ]]; then echo "Hexdump:"; xxd "$entry" 2>/dev/null || echo "Hexdump failed";
            elif [[ -L "$entry" ]]; then echo "Symlink target: $(readlink "$entry" 2>/dev/null || echo "readlink failed")"; fi
            echo "--- (end of entry $clean_entry) ---"; done)
        echo "--- disk_usage (size of data directory) ---"
        du -bs "$target_data_dir" 2>/dev/null | awk '{print $1}' || echo "(du failed)"
    else _harness_log "Dir '$target_data_dir' not found for gather."; echo "--- directory_listing ---"; echo "(dir N/A)"; echo "--- disk_usage ---"; echo "(dir N/A)"; fi
    return 0
}
...
test_diff() { 
    ...
    test_gather "expected" > "./expected.all" 2>&1 || { echo "Gather expected failed" > "./expected.all"; }
    test_gather "actual" > "./actual.all" 2>&1 || { echo "Gather actual failed" > "./actual.all"; }
    if diff -u "./expected.all" "./actual.all" > "./results.diff"; then
        _harness_log "No differences. PASS."; return 0;
    else echo "[HARNESS_INFO] Differences found. FAIL. See results.diff:" >&2; cat "./results.diff" >&2; return 1; fi
}
export -f test_diff
...
test_run_one() { 
    ...
    ( 
        ...
        for f in expected.stdout expected.stderr expected.return actual.stdout actual.stderr actual.return expected.all actual.all results.diff; do
            if [[ -f "./$f" ]]; then if ! mv "./$f" "$test_specific_archive_dir/"; then _harness_log "[ERROR] Failed mv ./$f to $test_specific_archive_dir/"; fi; fi
        done
